...
public abstract class IntegrationTestBase {
  ...
  protected void tearDown() {
    ...
    List<ServeEvent> unmatchedEvents = allServeEvents.stream().filter(e -> !e.getWasMatched()).toList();

    if (!unmatchedEvents.isEmpty()) {
      String details =
          unmatchedEvents.stream().map(e -> e.getRequest().getMethod() + " " + e.getRequest().getUrl()).toList()
              .toString();
    ...
    List<StubMapping> unusedStubs = allStubs.stream().filter(stub -> allServeEvents.stream()
            .noneMatch(event -> event.getStubMapping() != null && event.getStubMapping().getId().equals(stub.getId())))
        .toList();
